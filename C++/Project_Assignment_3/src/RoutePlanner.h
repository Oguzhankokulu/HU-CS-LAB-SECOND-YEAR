...
public:
    ...
    int longestDistanceCovered = 0;   // Tracks the longest distance of the route
    std::vector<int> route;       // Vector to store the route in order of visitation
    std::vector<int> longestRoute; // Vector to store the longest route.
    ...
    bool isInVector(int element, std::vector<int> vector) const;
    int numberOfNeighboors(int province);    // Checks the available neighboors number
